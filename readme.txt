...
* Bug fixes:
	* Fixed quotes in subpage titles breaking page parsing (e.g. "F48").
	* Fixed PHP 8.2+ deprecated dynamic property warnings.
	* Fixed catastrophic backtracking in regex patterns.
	* Fixed page 2+ not accessible due to redirect_canonical interference.
* Enhancements:
	* Added SEO rel=prev/rel=next links in head.
	* Added input sanitization and escaping across admin settings.
	* Added in-memory options caching to reduce DB queries.
	* Replaced $_REQUEST with explicit $_GET/$_POST checks.
	* Cleaned up dead code, Italian comments, and typos.
